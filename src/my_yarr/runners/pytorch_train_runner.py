...
class PyTorchTrainRunner(TrainRunner):
    ...
    def _step(self, i, sampled_batch):
        ...
        if "priority" in update_dict:
            priority = update_dict['priority'].cpu().detach().numpy() if isinstance(update_dict['priority'],
                                                                                    torch.Tensor) else np.numpy(
                update_dict['priority'])
        else:
            priority = None
    ...
    def start(self):
        ...
        for i in range(self._iterations):
            ...
            if self._target_replay_ratio is not None:
                ...
                # logging.info('wait for env_runner collecting enough samples')
                while True:
        ...
        if self._writer is not None:
            self._writer.close()
            logging.info('self._writer is not None:')
