...
EPISODE_FOLDER = 'episode2'
example_path = '/home/ylc/ARM/yt_data/real_demo/2/take_target_off_desk/variation0'
...
class My_Sence(Scene):
    ...
    def get_demo(self, record: bool = True,
        ...
        if record:
            position = self.get_observation().gripper_pose
            state = []
            state.extend([position[0], position[1], 0.156, 0, 0, position[3], -position[4]])
            state.extend([0.425, 0.05, 0.162, 0, 0, 0, 1])
            state.extend([position[0], position[1], 0.062, 0, 0, position[3], -position[4]])
            state.extend([position[0], position[1], 0.062, 0, 0, position[3], -position[4]])
            state.extend([position[0], position[1], 0.062, 0, 0, position[3], -position[4]])
            state.extend([0, 0, 0, 0, 0, 0])
            state.extend([position[0], position[1], 0.112, position[3], position[4], position[5], position[6]])
            state.extend([position[0], position[1], 0.062, position[3], position[4], position[5], position[6]])
            state.extend(position)
            self.task_low_dim_state = state
        ...
        while not done:
            ...
            if len(msg.status_list) == 2 and msg.status_list[
                1].text == "This goal has been accepted by the simple action server":
                done = True
...
def main():
    rospy.init_node('dataset_target_off')
    my_sence = My_Sence()
