...
def uint16_array_to_rgb_image(uint16_array, scale_factor=DEFAULT_RGB_SCALE_FACTOR, drop_blue=False):
    """
    Convert a uint16 array of values to an RGB image.
    Convert uint16 values to a fixed point representation where
    ...
    uint16 values are scaled so that the integer values cover
    the representable range of depths.
    This image representation should only use lossless compression.
    Args:
      uint16_array: Input array of uint16 depth values.
      scale_factor: Scale value applied to all uint16 values.
    ...
    """
    # Scale the uint16 array.
    uint16_array = uint16_array / 1000
    scaled_array = np.floor(uint16_array * scale_factor + 0.5)
    # Convert the array to uint32 type and clip to representable range.
    min_inttype = 0
    max_inttype = 2 ** 24 - 1
    scaled_array = np.clip(scaled_array, min_inttype, max_inttype)
