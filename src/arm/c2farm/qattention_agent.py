...
PICKLE = '%d.pkl'
path = '/home/ylc/ARM/yt_data/pcd'
path_image = '/home/ylc/ARM/yt_data/'
IMAGE_FORMAT = '%d.png'
REPLAY_BETA = 1.0
...
########################################
# def save(i, pcd, rgb):
#     pcd_rgb = {'pcd': pcd, 'rgb': rgb}
#     with open(os.path.join(path, PICKLE % i), 'wb') as f:
#         pickle.dump(pcd_rgb, f)
#     QFunction.i += 1
...
class QFunction(nn.Module):
    ...
    def _argmax_3d(self, tensor_orig):
        ...
        # indices = torch.cat([torch.div(torch.div(idxs,h,rounding_mode='floor'),d,rounding_mode='floor'), (torch.div(idxs,h,rounding_mode='floor')) % w, idxs % w], 1)
    ...
    def forward(self, x, proprio, pcd,
        ...
        ##############################################
        QFunction.i += 1
        QFunction.pcd = pcd_flat
        QFunction.rgb = image_features
        ##############################################
...
class QAttentionAgent(Agent):
    ...
    def update(self, step: int, replay_sample: dict) -> dict:
        ...
        with torch.no_grad():
            ...
            if with_rot_and_grip:
                target_q_tp1_rot_grip = self._get_value_from_rot_and_grip(q_rot_grip_tp1_targ,
                                                                          rot_and_grip_indicies_tp1)  # (B, 4)
                q_tp1_at_voxel_idx = torch.cat([q_tp1_at_voxel_idx, target_q_tp1_rot_grip], 1).mean(1, keepdim=True)

            q_target = (reward.unsqueeze(1) + (self._gamma ** self._nstep) * (
                    1 - terminal.unsqueeze(1)) * q_tp1_at_voxel_idx).detach()
            # q_target = torch.maximum(q_target, torch.zeros_like(q_target))
            q_target = torch.max(q_target, torch.zeros_like(q_target))
        ...
        if with_rot_and_grip:
            ...
            q_delta = torch.cat(
                [F.smooth_l1_loss(target_q_rot_grip, q_target.repeat((1, 4)), reduction='none'), q_delta], -1)
            qreg_loss += F.l1_loss(q_rot_grip, torch.zeros_like(q_rot_grip), reduction='none').mean(
                1) * self._lambda_rot_qreg
        ...
        # rgb = pImage.fromarray(color)
        # rgb.save(os.path.join(path_image, IMAGE_FORMAT % QFunction.i))
        #############################################################
    ...
    def act(self, step: int, observation: dict,
        ...
        #############################################################
        color = visualise_voxel(
            self._act_voxel_grid.detach().cpu().numpy(),
            self._act_qvalues.detach().cpu().numpy(),
            self._act_max_coordinate.detach().cpu().numpy())
        rgb = pImage.fromarray(color)
        rgb.save(os.path.join(path_image, IMAGE_FORMAT % QFunction.i))
        #############################################################
