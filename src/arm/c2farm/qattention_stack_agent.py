...
from get_image.src.arm.c2farm.qattention_agent import QAttentionAgent, QFunction
...
class QAttentionStackAgent(Agent):
    ...
    def act(self, step: int, observation: dict,
        ...
        for depth, qagent in enumerate(self._qattention_agents):
            ...
            # a = QFunction.pcd.cpu().numpy().reshape(-1, 3)
            # a = np.concatenate((a, attention_coordinate), axis=0)
            # print(attention_coordinate)
            # b = np.tile([0, 1, 0], (16384, 1))
            # b = np.concatenate((b, [[1, 0, 0]]), axis=0)
