...
from get_image.src.my_run_env import My_Run_Sence
...
class My_CustomRLBenchEnv(CustomRLBenchEnv):
    ...
    def reset(self) -> dict:
        ...
        obs = self.sence.get_observation()
    ...
    def step(self, act_result: ActResult) -> Transition:
        ...
        try:
            obs, reward, terminal = self.sence.step(action)
            if reward >= 1:
                success = True
